...
- acts as a fail-closed `tool_call` backstop: blocks Worker/Reviewer `mcp` and
  ...
  (`gitAuthorityBlockReason` — `MODE: write`-gated git ops, always-deny force-push/amend).
...
Pi Agent Profiles pin Lead to `openai-codex/gpt-5.6-sol`/`high` and
Worker/Reviewer/Supervisor to `openai-codex/gpt-5.6-luna`/`max`, validated
against the live catalog before writing.
...
2. Creates four role homes; copies each role's `AGENTS.md`, `settings.json`,
   ...
   brief at `$PI_CODING_AGENT_DIR/templates/TASK_BRIEF.md`, and links each
...
- `pi-orchestration/templates/TASK_BRIEF.md` — canonical brief template.
