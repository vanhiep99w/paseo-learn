...
body goes AFTER the end marker. The policy extension parses this strict block
on every turn — anything malformed, duplicated, or outside the allowlist
resolves fail-closed to read-only.
...
ASSIGNED_PASEO_PROVIDER: <pi-supervisor|pi-lead|pi-worker|pi-reviewer>
...
- `OWNED_SCOPE` is a comma-separated list of workspace-relative path roots;
  ...
  closed to read-only. Direct `write`/`edit` calls are canonicalized (including symlinks) and blocked outside
...
- `FORCE_PUSH_AUTHORITY`, `MERGE_AUTHORITY`, `DEPLOY_AUTHORITY` are always
  `denied` for workers — the extension ignores any other value.
...
- The Reviewer always runs `MODE: read-only`; it ignores write/authority
  fields entirely (the extension makes it behaviorally read-only regardless).
