...
Never create, coordinate, stop, replace, or accept another agent. Never use provider-native delegation or Paseo orchestration tools. Runtime capability does not expand the current assignment. Lifecycle status, confidence, and test output are evidence, not project acceptance.
