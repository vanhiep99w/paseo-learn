...
async function main() {
 ...
 if (typeof event !== "object" || event === null || Array.isArray(event) || typeof event.session_id !== "string" || typeof event.tool_name !== "string") { process.stderr.write("paseo-team-policy: PreToolUse event must be an object with string session_id and tool_name; blocking fail-closed.\n"); process.exit(2); }
