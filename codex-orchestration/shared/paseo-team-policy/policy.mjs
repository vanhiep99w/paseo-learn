...
export function supervisorCreateAgentBlockReason(input) {
	...
	if (!/^codex-lead\/[^/]+\/[^/]+/.test(provider)) {
		return `Supervisor create_agent is lead-recovery only: provider must be "codex-lead/<provider>/<model-id>" (got "${provider || "<missing>"}"). Peers and other providers are created by the Lead, never by the Supervisor.`;
	}
