#!/usr/bin/env node
/** Codex UserPromptSubmit hook: replace Peer authority state every turn. */
...
async function main() {
 if (process.env.PASEO_CODEX_ROLE?.trim().toLowerCase() !== "peer") process.exit(0);
 let event; try { const raw = await new Promise((resolve,reject)=>{let d="";process.stdin.setEncoding("utf8");process.stdin.on("data",c=>d+=c);process.stdin.on("end",()=>resolve(d));process.stdin.on("error",reject)}); event=raw.trim()?JSON.parse(raw):null; } catch { process.stderr.write("paseo-team-policy: invalid UserPromptSubmit event; blocking Peer turn fail-closed.\n"); process.exit(2); }
 if (!event || typeof event.session_id !== "string") { process.stderr.write("paseo-team-policy: UserPromptSubmit event has no session_id; blocking Peer turn fail-closed.\n"); process.exit(2); }
