...
`PASEO_CODEX_ROLE`, `PASEO_TEAM_LEAD_WRITE`, owned-scope; push/merge theo
`MODE: write` của brief (bất kỳ branch, kể cả branch đang
làm như `main`), force-push/amend luôn chặn. Hosted tools (ví dụ `WebSearch`) không đi qua hook path. `app-server`
