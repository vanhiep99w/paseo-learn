...
Claude installer tiếp tục dùng model đầu tiên/default được discover trên host;
role behavior vẫn đến từ custom provider riêng. Installer cố ý
...
```bash
PASEO_PI_AGENT_PROFILE_ROUTES_JSON='{"lead":{"model":"fixture/sol","thinkingOptionId":"high"},"worker":{"model":"fixture/luna","thinkingOptionId":"max"},"reviewer":{"model":"fixture/luna","thinkingOptionId":"max"},"supervisor":{"model":"fixture/luna","thinkingOptionId":"max"}}' \
  ./install pi
